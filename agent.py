#!/usr/bin/env python3
"""
agent.py — A web-browsing agent that uses an OpenAI-compatible LLM
to interact with websites based on a natural-language prompt.

Uses Playwright's accessibility snapshot (no custom JS injection) and
semantic locators (get_by_role, get_by_text, etc.) for interactions.

Usage:
    python agent.py --prompt "Find the price of the first product" \
